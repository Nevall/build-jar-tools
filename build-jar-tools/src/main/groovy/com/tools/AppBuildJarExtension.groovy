...
class AppBuildJarExtension {
    ...
    public static AppBuildJarExtension getConfig(Project project) {
        AppBuildJarExtension config =
                project.getExtensions().findByType(AppBuildJarExtension.class);
        if (config == null) {
            config = new AppBuildJarExtension();
        }
        return config;
    }
